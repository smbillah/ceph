			       ...
			       int uid, int gid)
{
  ...
  if (request->target) {
    ...
    ldout(cct, 20) << "make_request target is " << *target << dendl;
  } else {
    if (got_created_ino && (p = inode_map.find(vinodeno_t(created_ino, CEPH_NOSNAP))) != inode_map.end()) {
      target = p->second;
      ldout(cct, 20) << "make_request created, target is " << *target << dendl;
    } else {
		    ...
		    struct stat *attr, int uid, int gid)
{
  ...
  tout(cct) << vino << std::endl;
  tout(cct) << vnewparent << std::endl;
  tout(cct) << newname << std::endl;

  int r = _link(in, newparent, newname, uid, gid);
  if (r == 0) {
    fill_stat(in, attr);
    _ll_get(in);
  }
